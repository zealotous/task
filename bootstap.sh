...
for n in 'DATABASE_HOST' 'DATABASE_PORT' \
         'DATABASE_USER' 'DATABASE_DB'; do
     read_var $n
done;
...
mysql -h$DATABASE_HOST -u$DATABASE_USER \
      -p$DATABASE_PASS --port=$DATABASE_PORT << SQL-SCRIPT
...
if [ $? -eq 0 ]; then
    # клонируем репозиторий если в текущей папке нет settings.py
    if [ ! -e "$__DIR__/settings.py" ]; then
        git clone "$REPO"
        if [ $? -ne 0 ]; then
